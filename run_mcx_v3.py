#!/usr/bin/env python3
"""
MCX Re-optimisation v3 — Capital-normalised MaxDD
===================================================
Bug fixed: MaxDD now computed as (peak-trough)/CAPITAL × 100
instead of (peak-trough)/peak × 100.

With equity-relative DD:  Silver DD=120%, Gold DD=42%  → wrongly failed 5% filter
With capital-relative DD: Silver DD~5-12%, Gold DD~4-8% → correctly passes

Target: PF ≥ 2.5, MaxDD(capital) < 15%, min_trades ≥ 20
Fallback: PF ≥ 2.0 if none pass
"""
...
CAPITAL    = 500_000   # ₹5L capital base for DD normalisation
MIN_TRADES = 20
TARGET_PF  = 2.5
MAX_DD     = 15.0      # 15% of CAPITAL (₹75k max drawdown)
...
# ── backtest with capital-normalised DD ─────────────────────────────────────
def backtest_v3(closes, ef_arr, es_arr, rsi_arr, atr_arr, adx_arr,
    ...
    for i in range(2, n):
        ...
        if pos != 0:
            ...
            if exit_p is not None:
                ...
                # ── KEY FIX: DD relative to CAPITAL not to peak equity ──────
                dd_pct = (peak - equity) / CAPITAL * 100
...
for name, df in raw.items():
    ...
    print(f"  Indicators pre-computed ({len(ema_cache)} EMA pairs)")
    ...
    for (ef,es),rb,rs,adx_th,atr_sl,trail in itertools.product(
        ...
        if trail is None:
            for atr_tp in ATR_TP:
                tested += 1
                r = backtest_v3(closes, ef_arr, es_arr, rsi_arr, atr_arr, adx_arr,
                                rb, rs, adx_th, atr_sl, atr_tp=atr_tp)
                if r and r["trades"] >= MIN_TRADES and r["pf"] >= TARGET_PF and r["dd"] <= MAX_DD:
    ...
    print(f"  {'EMAf':>4} {'EMAs':>5} {'RSIb':>4} {'RSIs':>4} {'ADX':>4} "
          f"{'SL×':>4} {'TP×':>5} {'Trail':>5} | {'PF':>6} {'DD%':>5} {'WR%':>5} {'#T':>4} {'Net':>12}")
    print("  " + "-"*72)
    for r in results[name]:
        ...
        print(f"  {r['ema_f']:>4} {r['ema_s']:>5} {r['rsi_buy']:>4} {r['rsi_sell']:>4} "
              ...
              f"{r['pf']:>6.2f} {r['dd']:>5.1f} {r['wr']:>5.1f} {r['trades']:>4} "
              f"₹{r['net']:>11,.0f}")

with open("/Users/mac/sksoopenalgo/openalgo/mcx_v3_results.json","w") as f:
    json.dump(results, f, indent=2)
print("\n✅ MCX v3 results saved to mcx_v3_results.json")
