#!/usr/bin/env python3
"""
SensexWeeklyStrategy - SENSEX Weekly Options (OpenAlgo Web UI Compatible)
SENSEX Iron Condor Strategy for Weekly Income on BSE (BFO).
...
# ===========================
STRATEGY_NAME = os.getenv("STRATEGY_NAME", "sensex_weekly_strategy")
...
SL_PCT = float(os.getenv("SL_PCT", "40"))        # % of Net Credit
TP_PCT = float(os.getenv("TP_PCT", "50"))         # % of Net Credit
...
class NetCreditPositionTracker(OptionPositionTracker):
    ...
    def should_exit(self, chain):
        ...
        # Check SL (e.g. -40%)
        # Note: self.sl_pct is positive number (e.g. 40)
...
class SensexWeeklyStrategy:
    ...
    def _close_position(self, chain, reason):
        ...
        try:
            ...
            # Assume success and clear tracker
            # In production, check response status
            if res.get("status") == "success":
                self.tracker.clear()
            else:
                self.logger.error(f"Exit failed: {res.get('message')}")
                # Force clear if needed or retry logic
                # self.tracker.clear() # Optional: force clear to avoid loop
    ...
    def _open_position(self, chain, entry_reason):
        ...
        for cfg in legs_config:
            ...
            if found_item:
                ...
                # Leg for API (can use offset or symbol, using offset for robust execution)
                api_legs.append({
                    "offset": offset,
                ...
                # Leg for Tracker (needs symbol and entry price)
        ...
        try:
            ...
            if res.get("status") == "success":
                ...
                # Add to Tracker
                # Note: Entry prices are from Chain snapshot (approximate).
                # Ideally use Order Book fills.
        ...
        while True:
            try:
                ...
                if self.tracker.open_legs:
                    ...
                    # Additional Override: Friday 2PM Exit
                    ist_offset = timezone(timedelta(hours=5, minutes=30))
                    now = datetime.now(ist_offset)
                    ...
                    # Additional Override: EOD 3:15 PM Exit
                    ...
                    # Using should_exit logic indirectly or re-calc
                    # Just generic log
                ...
                if self.entered_today:
                    # Log occasionally
                    # self.logger.info("Daily limit reached (entered_today=True).")
                    time.sleep(SLEEP_SECONDS)
                    continue
