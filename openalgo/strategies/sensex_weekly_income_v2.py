#!/usr/bin/env python3
"""
SensexWeeklyIncomeV2 - SENSEX Weekly Options (OpenAlgo Web UI Compatible)
Production-Ready SENSEX Iron Condor Strategy for Weekly Income on BSE (BFO).
...
Strategy Logic:
- Enters after 10:00 AM on weekdays if Straddle Premium > 500.
- Sells OTM2 CE/PE (Strangle), Buys OTM4 CE/PE (Wings) for protection.
- Risk Management: SL 35% of Net Credit, TP 45% of Net Credit.
- Max Hold: 45 minutes.
- Friday Expiry: Hard Exit at 14:00 to avoid gamma risk.
- Daily Limit: 1 Trade per day.
- Defensive: Normalizes Exchange codes for SENSEX/BANKEX to BSE_INDEX/BFO.
"""
import os
import sys
import time
...
# Check for utils in the same directory (strategies/utils)
utils_dir = os.path.join(script_dir, "utils")

if os.path.exists(os.path.join(utils_dir, "optionchain_utils.py")):
    strategies_dir = script_dir
else:
...
# ===========================
STRATEGY_NAME = os.getenv("STRATEGY_NAME", "sensex_weekly_income_v2")
...
STRIKE_COUNT = int(os.getenv("STRIKE_COUNT", "15")) # Wider strike count for safety
...
MAX_ORDERS_PER_DAY = int(os.getenv("MAX_ORDERS_PER_DAY", "1")) # Strategy constraint: 1 trade/day
...
class NetCreditPositionTracker(OptionPositionTracker):
    ...
    def should_exit(self, chain):
        ...
        for leg in self.open_legs:
            ...
            # We assume equal quantities for simplicity in % calculation
            # or sum up total value (Price * Qty * LotSize is implied if LotSize is constant)
            # Since Qty is handled per leg, we multiply by Qty.
        ...
        # If we collected 100, and cost to close is 80, PnL is +20.
        # If cost to close is 120, PnL is -20.
        ...
        # Check SL (e.g. -35%)
        # Note: self.sl_pct is positive number (e.g. 35)
...
class SensexWeeklyIncomeV2:
    ...
    def ensure_expiry(self):
        ...
        # If expiry date is manually set in environment and hasn't expired, respect it.
        # But here we want auto-rollover logic.
        # If user set EXPIRY_DATE manually in config, we use it.
        if EXPIRY_DATE:
             self.expiry = EXPIRY_DATE
             return
    ...
    def _open_position(self, chain, entry_reason):
        ...
        # We use offset labels directly if API supports smart offsets,
        # but for tracking we need symbols.
        # OptionChainClient.optionsmultiorder supports offsets ("OTM2") directly.
        # But we need symbols for the Tracker.

        # 1. Resolve Symbols from Chain manually to populate tracker correctly
        legs_config = [
        ...
        for cfg in legs_config:
            offset = cfg["offset"]
            otype = cfg["option_type"].lower() # 'ce' or 'pe'
        ...
        try:
            ...
            if res.get("status") == "success":
                ...
                self.tracker.add_legs(resolved_legs, entry_prices, side="SELL") # Iron Condor is Credit = SELL logic
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # 4. Exit Management (Always check before Entry)
                ...
                # Strategy Logic: Straddle Premium Check
