...
def sync_running_processes():
    """Detect running strategy processes and sync config state"""
    with PROCESS_LOCK:
            ...
            try:
                ...
                for key, strategy_id in strategy_name_map.items():
                    if key in cmd_str and 'strategy' in cmd_str:
                        # Check if this strategy is in config
                        if strategy_id in STRATEGY_CONFIGS:
                            config = STRATEGY_CONFIGS[strategy_id]
                            pid = proc.info['pid']
                            
                            # Check if PID matches or if config shows not running but process exists
                            if config.get('pid') != pid or not config.get('is_running', False):
                                # Verify process is actually running
                                if psutil.pid_exists(pid):
                                    STRATEGY_CONFIGS[strategy_id]['is_running'] = True
                                    STRATEGY_CONFIGS[strategy_id]['pid'] = pid
                                    synced_count += 1
                                    logger.info(f"Synced running process for {strategy_id} (PID: {pid})")
                        break
        ...
        if synced_count > 0:
            save_configs()
            logger.info(f"Synced {synced_count} running processes")


def cleanup_dead_processes():
    """Clean up strategies with dead processes and sync running processes"""
    with PROCESS_LOCK:  # Thread-safe operation
        dead_strategies = []
        ...
        # Also sync running processes that might not be in RUNNING_STRATEGIES
        sync_running_processes()
...
def index():
    ...
    # Reload configs to pick up manually added strategies
    load_configs()
    cleanup_dead_processes()  # This now also syncs running processes
